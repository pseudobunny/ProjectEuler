...
impl Grid {
    fn create_grid(d: &[u32], r: usize, c: usize) -> Grid {
        Grid{ data: d.to_vec(), rows: r, columns: c }
    }
...
fn check_direction(grid: &Grid, x0: i32, y0: i32, vert_dir: i32, hor_dir: i32, dist: i32) -> u64 {
    ...
    if end_x < 0 || end_x > 19 || end_y < 0 || end_y > 19 {
        return 0;
    }

    let mut product : u64 = 1;
    for i in 0..dist {
        product *= grid.get((x0+i*hor_dir) as usize, (y0+i*vert_dir) as usize) as u64;
    }
...
fn max_in_each_direction(grid: &Grid, x0: i32, y0: i32, dist: i32) -> u64 {
    ...
    for h in 0..3 {
        for k in 0..3 {
            let h_adj = h-1;
            let k_adj = k-1;
            if h_adj == 0 && k_adj == 0 { continue; }

            curr_prod = check_direction(grid, x0, y0, h_adj, k_adj, dist);
...
fn main() {
    ...
    for i in 0..20 {
        for j in 0..20 {
            curr_prod = max_in_each_direction(&grid, i, j, dist);
