...
fn check_set_viable(n_v: &[u64]) -> bool {
    ...
    required = HashSet::<u64>::from_iter(required.iter().cloned()).iter().copied().collect();
    ...
    let all_options = HashSet::<u64>::from_iter(optional.iter().flat_map(|t| once(t.0).chain(once(t.1))))
        .iter()
        .copied()
        .collect::<Vec<u64>>();
    ...
    let best_options = (1..=all_options.len())
        ...
        .map(|v| extend_set(&v))
        .find(|v| optional.iter().all(|p| v.contains(&p.0) || v.contains(&p.1)));
    ...
    let len = required.len() - if required.contains(&6) && required.contains(&9) { 1 } else { 0 };
